...
class Test_Build(unittest.TestCase):
    ...
    def test_errors_in_log(self):
        ...
        """        
        with open(os.path.join(os.getcwd(), 'aview.log'), 'r') as fid:
            text = fid.read()
...
class Test_BuildInUNC(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # Write the string file
        string_file = string.write_to_file(TEST_WORKING_DIRECTORY_UNC)
        
        # Create DrillSolverSettings object
        solver_settings = adripy.DrillSolverSettings('baseline_settings')

        # Write the solver settings file
        solver_settings_file = solver_settings.write_to_file(TEST_SOLVER_SETTINGS_NAME, directory=TEST_WORKING_DIRECTORY_UNC)

        # Build model
        adripy.build(string_file, solver_settings_file, TEST_WORKING_DIRECTORY_UNC)

    def test_aview_cmd_is_gone(self):
        """Tests that aview.cmd does not exist in the cwd.
        
        """
        self.assertFalse(os.path.exists(os.path.join(os.getcwd(), 'aview.cmd')))

    def test_cd_then_build_is_gone(self):
        """Tests that cd_then_build.cmd does not exist in the cwd.

        """
        self.assertFalse(os.path.exists(os.path.join(os.getcwd(), '_cd_then_build.cmd')))

    def test_adm(self):
        """Tests that the adm was created
        """
        self.assertTrue(os.path.exists(os.path.join(TEST_WORKING_DIRECTORY_UNC, TEST_STRING_NAME + '.adm')))

    def test_acf(self):
        """Tests that the acf was created
        """
        self.assertTrue(os.path.exists(os.path.join(TEST_WORKING_DIRECTORY_UNC, TEST_STRING_NAME + '.acf')))
    ...
    def tearDownClass(cls):
        adripy.remove_cdb_from_cfg(TEST_DATABASE_NAME, os.environ['ADRILL_USER_CFG'])
        # Remove all the files in the working directory
        # for file in glob.glob(os.path.join(TEST_WORKING_DIRECTORY_UNC, '*')):
