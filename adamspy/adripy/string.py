...
class DrillString():
    ...
    def read_from_file(cls, filename):
        ...
        tiem_orbit_data = read_TO_file(filename)

        drill_string = cls('','','')
        
        # Extract the DrillString parameters from the TO dictionary        
        drill_string._get_params_from_TO_data(tiem_orbit_data, filename) #pylint: disable=protected-access
    ...
    def _get_params_from_TO_data(self, tiem_orbit_data, tiem_orbit_filename):
        ...
        for param in self._SCALAR_PARAMETERS + self._ARRAY_PARAMETERS + self._FILENAME_PARAMETERS:
            ...
            if param in self._FILENAME_PARAMETERS:
                if not os.path.isabs(param_value) and param_value == get_full_path(param_value):
                    param_value = os.path.join(os.path.split(get_cdb_path(tiem_orbit_filename))[0], param_value)
